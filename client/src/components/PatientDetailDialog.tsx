...
import { ClipboardList, Activity, TrendingUp, Clock, Download, Brain } from "lucide-react";
...
export function PatientDetailDialog({
  ...
  const [loadingMLResults, setLoadingMLResults] = useState(true);
  ...
  useEffect(() => {
    ...
    const mlResultsQuery = query(
      collection(db, "mlAnalysisResults"),
      where("patientId", "==", patient.uid),
      orderBy("timestamp", "desc"),
      limit(20)
    );

    const unsubscribeMLResults = onSnapshot(
      mlResultsQuery,
      (snapshot) => {
        ...
        setLoadingMLResults(false);
      },
      (error) => {
        console.error("Error fetching ML analysis results:", error);
  ...
  const handleDownloadReport = () => {
    try {
      ...
      if (mlAnalysisResults.length > 0) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text("ML Performance Analysis", 14, yPos);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(12);
        yPos += 8;

        // Show latest 3 ML results
        const latestResults = mlAnalysisResults.slice(0, 3);
        latestResults.forEach((result, index) => {
          ...
          doc.setFont("helvetica", "bold");
          doc.setFontSize(11);
          doc.text(`${result.exerciseName}`, 14, yPos);
          doc.setFont("helvetica", "normal");
          doc.setFontSize(10);
          yPos += 5;
          doc.text(
            [
              ...
              `Recovery Stage: ${result.recovery_stage}`,
        ...
        <div className="space-y-6 mt-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                AI-powered exercise performance evaluation and recovery stage assessment
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingMLResults ? (
              ...
              ) : mlAnalysisResults.length === 0 ? (
                <div className="text-center py-12">
                  <Brain className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
                  <p className="text-muted-foreground">No ML analysis results yet</p>
                  <p className="text-xs text-muted-foreground mt-2">
                    Results will appear after patients complete exercises with device recordings
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {mlAnalysisResults.map((result) => (
                    <div
                      key={result.id}
                      className="p-6 border rounded-lg bg-gradient-to-r from-primary/5 to-primary/10 hover:from-primary/10 hover:to-primary/15 transition"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h4 className="text-lg font-semibold">{result.exerciseName}</h4>
                            <Badge
                              variant={
                          ...
                          <p className="text-xs text-muted-foreground mb-3">
                            Analyzed {new Date(result.timestamp).toLocaleString()} • {result.sampleCount} samples
                          </p>
                        </div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xl font-semibold">{result.recovery_stage}</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            {result.recovery_stage.includes("Stage 1")
                              ? "Early recovery phase - focus on ROM"
                              : result.recovery_stage.includes("Stage 2")
                              ? "Strengthening phase - progressive loading"
                              : "Advanced mobility - functional training"}
                          </p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-xs uppercase text-muted-foreground tracking-wide">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            {result.status === "Ready"
                              ? "Patient ready for next stage"
                              : "Continue current rehabilitation stage"}
                          </p>
                        </div>
                      </div>
                    </div>
